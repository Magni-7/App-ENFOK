...
export default async function RestablecerPage({ searchParams }: RestablecerPageProps) {
  const { token, error } = await searchParams;

  const resetToken = token ? await prisma.passwordResetToken.findUnique({ where: { token } }) : null;
  const isValid = resetToken && !resetToken.usedAt && resetToken.expiresAt > new Date();

  return (
    <div className="mx-auto flex max-w-sm flex-col gap-6">
      <div>
        <h1 className="font-serif text-2xl font-semibold tracking-tight">Restablecer contraseña</h1>
      </div>
